		...
		for _, match := range matches {
			...
			for _, pos := range positions {
				if checkPosition(pos, lines, numLines, lineLength) {
					...
					// don't break this time because we want to associate the
					// part number with any adjacent *
					l := location{pos[0], pos[1]}
					gearMap[l] = append(gearMap[l], num)
				}
			}
		}
	}
